...
export function VideoInformationPopup({
  ...
  return (
    <motion.div
      className="h-full overflow-y-scroll bg-neutral-950/70 pt-12 backdrop-blur-3xl backdrop-brightness-200 backdrop-saturate-200 md:pt-0"
    ...
    >
      <motion.div className="grid w-full grid-cols-[1fr_auto_1fr]">
        <div />
        <motion.div className="w-container relative mx-auto max-w-7xl md:p-10">
          <motion.img
            initial={{ opacity: 0 }}
          ...
          <motion.video
            ...
            className="absolute aspect-video rounded-xl object-cover ring-2 shadow-lg shadow-neutral-950 ring-white/5 ring-inset md:w-[calc(100%-5rem)]"
            src={videoURL}
            poster={image.src}
            transition={{
          ...
          <div className="flex w-full flex-col flex-wrap justify-between gap-8 md:flex-row md:items-center">
            <motion.span
              className="pointer-events-none z-50 order-2 flex h-5 items-center text-2xl font-semibold shadow-neutral-950 md:order-1"
  ...
  return (
    <motion.div
      onClick={() => {
        ...
        setTimeout(() => {
          globalOpenVideo.value = video
        }, POPUP_DURATION - 100)
      }}
      className="group z-20 ml-auto flex cursor-pointer flex-col text-white"
    >
      <div className="flex gap-4">
        <div className="flex flex-col text-right">
          <span className="text-xs uppercase">{side}</span>
          <span className="text-md font-semibold text-white shadow-black/50">
            {video.title}
          </span>
        </div>
        <img
          src={video.image.src}
          className="spring-bounce-50 spring-duration-300 max-h-16 rounded-md shadow-2xl transition-transform group-hover:scale-105"
        />
      </div>
    </motion.div>
  )
